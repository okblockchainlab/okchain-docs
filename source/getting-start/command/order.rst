...
Dex supports order placing and cancellation functions. Commands are as
follows:
...
``orderId`` indicates order id, which is the only id automatically
generated on the application layer

2. Operation query:
-------------------

Dex supports various functions including order query, depthbook and
users' order books. Commands are as follows:

.. code:: bash

    okchaincli query order

Secondary sub-commands mainly include the following functions ### 2.1
